...
fn main() {
    let args = Args::parse();
    scan_process_memory(args.pid as u32, args.pattern);
}

fn scan_process_memory(process_id: u32, pattern: String) {
    ...
    unsafe {
        ...
        let memory_regions = scan_memory_regions(process_handle);
        ...
        for region in memory_regions {
            ...
            //);
            if let Ok(found) = scan_memory_region(process_handle, &region, pattern.as_str()) {
...
fn pattern_to_bytes(pattern: &str) -> Result<Vec<u8>, ParseIntError> {
...
fn scan_memory_regions(process_handle: HANDLE) -> Vec<MemoryRegion> {
    ...
    unsafe {
        ...
        loop {
            ...
            let is_readable = mbi.Protect & PAGE_READONLY != 0 || mbi.Protect & PAGE_READWRITE != 0;
